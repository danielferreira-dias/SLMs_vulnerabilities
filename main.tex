...
\begin{tikzpicture}[
    % Define colors
    mainblue/.style={fill=blue!8, draw=black!70, line width=0.5pt},
    maingray/.style={fill=gray!10, draw=black!70, line width=0.5pt},
    excludered/.style={fill=red!5, draw=black!50, line width=0.5pt},
    finalgreen/.style={fill=green!8, draw=black!70, line width=0.5pt},
    % Node styles
    dbbox/.style={rectangle, rounded corners=3pt, mainblue, minimum width=2.4cm, minimum height=1cm, text centered, text width=2.2cm, font=\scriptsize},
    widebox/.style={rectangle, rounded corners=3pt, mainblue, minimum width=4.5cm, minimum height=0.8cm, text centered, text width=4.3cm, font=\scriptsize},
    otherbox/.style={rectangle, rounded corners=3pt, maingray, minimum width=2cm, minimum height=0.8cm, text centered, text width=1.8cm, font=\scriptsize},
    excludebox/.style={rectangle, rounded corners=3pt, excludered, minimum width=2.6cm, minimum height=0.9cm, text centered, text width=2.4cm, font=\scriptsize},
    finalbox/.style={rectangle, rounded corners=3pt, finalgreen, minimum width=4.5cm, minimum height=0.9cm, text centered, text width=4.3cm, font=\scriptsize\bfseries},
    stagelabel/.style={rectangle, rounded corners=2pt, fill=black!80, text=white, font=\scriptsize\bfseries, minimum width=1.6cm, minimum height=0.5cm, rotate=90, anchor=center},
    myarrow/.style={-{Stealth[length=2.5mm, width=2mm]}, line width=0.8pt, black!70}
]

% Stage labels (left side) with background
\node[stagelabel] at (-4.5, 1.0) {Identification};
\node[stagelabel] at (-4.5, -1.0) {Screening};
\node[stagelabel] at (-4.5, -3.0) {Eligibility};
\node[stagelabel] at (-4.5, -5.0) {Inclusion};

% Subtle stage separator lines
\draw[black!20, line width=0.5pt] (-3.6, 0.0) -- (4.5, 0.0);
\draw[black!20, line width=0.5pt] (-3.6, -2.0) -- (4.5, -2.0);
\draw[black!20, line width=0.5pt] (-3.6, -4.0) -- (4.5, -4.0);

% === IDENTIFICATION ===
\node[dbbox] (db1) at (-2.3, 1.7) {arXiv\\{\footnotesize (n = 26)}};
\node[dbbox] (db2) at (0, 1.7) {Semantic Scholar\\{\footnotesize (n = 43)}};
\node[dbbox] (db3) at (2.3, 1.7) {ACL Anthology\\{\footnotesize (n = 3)}};

\node[otherbox] (other) at (4, 0.9) {Other sources\\{\footnotesize (n = 3)}};

\node[widebox] (total) at (0, 0.5) {Records identified from databases\\{\footnotesize (n = 72) + other sources (n = 3) = \textbf{75 total}}};

% Arrows from databases to total
\draw[myarrow] (db1.south) -- ++(0,-0.15) -| ($(total.north)+(-1.2,0)$);
\draw[myarrow] (db2.south) -- (total.north);
\draw[myarrow] (db3.south) -- ++(0,-0.15) -| ($(total.north)+(1.2,0)$);
\draw[myarrow] (other.south) -- ++(0,-0.1) -| ($(total.north east)+(-0.3,0)$);

% === SCREENING ===
\node[widebox] (dedup) at (0, -0.7) {Records after duplicates removed\\{\footnotesize (n = 68; duplicates removed: 7)}};

\node[excludebox] (exc1) at (3.5, -1.4) {Excluded at screening\\{\footnotesize (n = 3)}\\{\tiny Pre-2020: 2, Out of scope: 1}};

\draw[myarrow] (total.south) -- (dedup.north);
\draw[myarrow] (dedup.south) -- ++(0,-0.25) -| (exc1.north);

% === ELIGIBILITY ===
\node[widebox] (fulltext) at (0, -2.7) {Full-text articles assessed for eligibility\\{\footnotesize (n = 65)}};

\node[excludebox] (exc2) at (3.5, -3.3) {Excluded at full-text\\{\footnotesize (n = 0)}};

\draw[myarrow] (dedup.south) -- ++(0,-0.25) -- ++(0,-0.55) -- (fulltext.north);
\draw[myarrow] (fulltext.east) -- (exc2.west);

% === INCLUSION ===
\node[finalbox] (final) at (0, -4.8) {Studies included in systematic review\\{\footnotesize (n = 65)}};
